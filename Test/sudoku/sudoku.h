...
public:
	...
	bool find_unassigned(char** board, int& row, int& col) const;

	bool solve(char** board) const;

	bool find_solutions(char** board, int& solution_num) const;

	bool read_board(std::ifstream& infile, char** board) const;

	void solve_games() const;

	bool get_next_nonblank(int& row, int& col, bool** skip_place) const;

	void generate_games() const;

	bool is_legal(char** board, int row, int col, int num) const;
};